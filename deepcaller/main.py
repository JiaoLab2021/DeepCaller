...
# Target depth (X) used for per-chromosome downsampling when --downsample is set
PLOIDY_DOWNSAMPLE_TARGET = {
    4: 50,
    6: 80,
}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f"DeepCaller {__version__} - Tool for identifying polyploidy small variation sites",
    ...
    io_group.add_argument("-S", "--sample", default="SAMPLE", help="Sample name/id")
    ...
    processing.add_argument(
        ...
        action="store_true",
        help=(
            "Downsample each chromosome's BAM to a target depth "
            "(ploidy 4: 50X, ploidy 6: 80X) if the genome-wide depth exceeds it. "
            "Chromosomes already below the target are left unchanged"
        ),
    )
    ...
    if not args.sample or any(c.isspace() for c in args.sample):
        sys.exit(f"[ERROR] --sample must be non-empty and contain no whitespace, got: {args.sample!r}")
    ...
    # ============================================================
    # STEP 5: BAM FILTERING & DEPTH CALCULATION
    # ============================================================
    try:
        print("\n" + "=" * 80)
        print("BAM FILTERING & DEPTH CALCULATION".center(80))
        print("=" * 80)
        ...
        chrom_depth_map, bam_depth = summarize_genome_depth(work_dir, all_chroms)
        print(f"* Use {bam_depth}X as the sequencing depth")

        print("[OK] BAM filtering & depth calculation complete!")

    except Exception as e:
        sys.exit(f"\n[ERROR] BAM filtering/depth calculation failed:\n{str(e)}")

    # ============================================================
    # STEP 6: DOWNSAMPLING (optional)
    # ============================================================
    final_bam_paths = {chrom: os.path.join(work_dir, f"temp_{chrom}.bam") for chrom in all_chroms}

    if args.downsample:
        try:
            print("\n" + "=" * 80)
            print("DOWNSAMPLING".center(80))
            print("=" * 80)

            target_depth = PLOIDY_DOWNSAMPLE_TARGET[args.ploidy]

            if bam_depth > target_depth:
                print(f"* Genome-wide depth ({bam_depth}X) exceeds target ({target_depth}X), downsampling per chromosome")

                downsample_tasks = []
                for chrom in all_chroms:
                    length_i, depth_i = chrom_depth_map.get(chrom, (None, None))
                    if depth_i is not None and depth_i > target_depth:
                        frac = target_depth / depth_i
                        downsample_tasks.append((work_dir, chrom, frac, args.seed))

                if downsample_tasks:
                    with Pool(processes=num_threads) as pool:
                        results = pool.map(downsample_chrom, downsample_tasks)

                    for chrom, sampled_bam, new_depth in results:
                        final_bam_paths[chrom] = sampled_bam
                        length_i, _ = chrom_depth_map[chrom]
                        chrom_depth_map[chrom] = (length_i, new_depth)

                    bam_depth = weighted_genome_depth(chrom_depth_map)
                    print(f"* Recalculated genome-wide depth after downsampling: {bam_depth}X")
                else:
                    print("* No individual chromosome exceeded the target depth, nothing to downsample")
            else:
                print(f"* Genome-wide depth ({bam_depth}X) does not exceed target ({target_depth}X), skipping downsampling")

            print("[OK] Downsampling complete!")
